...
`docs/STATE_SNAPSHOT.md` is generated by `node tools/state_snapshot.mjs` from
those files (plus `Enemy.ts`, `RunState.ts`, `EliteAffixes.ts`) — never edit
it by hand, edits are overwritten on the next run. Any change to balance
numbers must regenerate and commit it; `npm run qc` runs
`node tools/state_snapshot.mjs --check` as a static gate and fails if the
committed snapshot doesn't match the code.

...
## 2026-07-30 — Temporary per-weapon visuals and compact documentation

- **Target**: Character equipment visuals, projectile/slash effects, and
  project documentation.
- **Intent**: Make every currently defined weapon recognizable in play without
  replacing the finished default character illustration, while reducing the
  context required to start a new implementation task.
- **Result**: The default M1911 + katana keeps the original 27-frame art.
  Other loadouts composite temporary per-weapon sprites over the weapon-neutral
  character body sheet. Each gun selects its own projectile cell and each sword
  selects its own close-range effect cell. The old large design log became this
  archive; current status, handoff, index, and unresolved decisions are split
  into compact documents.
- **Follow-up**: Replace temporary sprites with final 112×64, 27-frame weapon
  motion sheets when art is delivered. No gameplay values changed.

## 2026-07-30 — 픽셀 스프라이트 렌더링 계약 통일

- 텍스처 파일과 절차 생성 캔버스가 모두 최근접 필터, mipmap 비활성화,
  sRGB 색공간을 사용하도록 공통 유틸리티로 묶었다.
- 캐릭터·몬스터·상호작용 오브젝트·잔상·사망 연출은 하단 중앙 기준점과
  원본 종횡비를 유지하는 월드 높이 계산을 공유한다.
- 기존 캐릭터·몬스터 원본, 애니메이션 프레임 수, 개체별 표시 크기는 바꾸지 않았다.
- 상호작용 오브젝트의 발광 복제본도 원본 종횡비를 유지하므로 직사각형 오브젝트가
  정사각형으로 찌그러져 빛나던 표현을 바로잡았다.
- `npm run qc` 22단계와 에셋 무결성 검사를 통과했으며,
  `qc-out/contact.png`에서 캐릭터·몬스터·오브젝트 기준점과 비율을 확인했다.
...
## 2026-07-30 — Rifle tuning

- Scoped rifle only: damage 25→50, magazine 10→5, pierce 2→3, shot interval
  0.28s→0.56s, and reload 1.6s→3.2s.
- Projectile speed, spread, and all other weapons are unchanged.

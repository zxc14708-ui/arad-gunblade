...
- `급소 간파`(치명타 +12%)를 반복 획득하면 확률이 100%를 넘김
- 검 쿨타임도 동일 구조

**확인 필요**: `Player.recompute()`에 상한이 걸려 있는지 아직 검증 안 됨.
이미 방어돼 있다면 이 항목은 무효.
...
기본값은 쿨타임 대비 무적 시간이 적정하지만, 특성이 붙으면 무너진다.

- `연막 회피` + `⭐질풍` 조합 → 무적 가동률 약 76%
- `연막 회피`를 한 번 더 획득 → **무적 시간이 쿨타임을 초과, 상시 무적**
...
- `src/entities/Player.ts` — `recompute()`의 스탯 상한 유무 (B1·B2 판단에 직결)

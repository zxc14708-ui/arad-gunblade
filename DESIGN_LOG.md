...
### B5. 골드가 상점 용량을 크게 초과해 후반부엔 쓸 곳이 없다 (3차 실측 — 설계 판단 필요, 미해결로 유지)

**2026-07-28 3차 실측에서 구조적 결함 2건을 발견했다. 이 두 건이 1·2차 측정의
전제 자체를 무효화하므로, 새 수치를 내기 전에 반드시 짚는다.**

1. **마을 상점은 실제로는 도달 불가능하다.** `Game.ts`의 `enterTown()`과
...
2. **분수 유료화(3단계)는 단일 런 안에서 사실상 죽은 코드다.** 분수는
   던전 상점방에만 배치되고(`loadRoom()`), 상점방은 런당 정확히 1개다
   (`RunState.generateMap()`의 `shopIndex`). 그런데 분수는 `run.isObjectUsed
   ('fountain')`로 **방(room) 단위 1회 제한**이 걸려 있고, 이 제한은 같은
   방을 몇 번을 재방문해도 풀리지 않는다(`usedObjects`가 런 내내 유지되는
   `RoomNode`에 저장됨). 상점방이 1개뿐이므로 분수도 런당 정확히 1회만
   트리거되고, 그 1회는 항상 `fountainFreeUsed`가 아직 false인 "첫 사용
   무료" 경로로만 실행된다 — 유료 경로(`fountainPrice` 적용)는 코드는
   맞게 동작하지만(직접 호출로 로직 자체는 재확인함) **같은 런에서 다시
   트리거될 방법이 없다.** `window.__game`으로 직접 확인: 상점방에서 분수
   1회 사용(무료, 회복 정상) → 같은 방문 중 재사용 시도(막힘, 골드·체력
   불변) → 방을 나갔다 같은 상점방에 재입장 후 재사용 시도(여전히 막힘).

이 두 가지는 **설계 판단이 필요한 문제라 여기 기록만 하고 직접 고치지
않았다** — 마을 상점의 존재 의의(마을에 골드를 가져올 방법을 새로
만들 것인지, 마을 상점을 없앨 것인지), 분수 룸을 스테이지당 여러 개로
늘릴지, 방당 제한을 런당 제한으로 바꿀지는 기획 결정이 먼저 필요하다.

**위 두 결함을 반영한 실제 도달 가능 경제 재측정** (4000회 몬테카를로,
...
**결론**: 던전 제련소(2단계)는 실제로 큰 흡수력을 낸다 — 마을 상점이
애초에 도달 불가능했다는 걸 감안하면, 2단계가 사실상 B5의 핵심 해법
역할을 하고 있다. 반면 분수 유료화(3단계)는 현재 룸 구조에서는 흡수력
기여가 0이라 사실상 목적을 달성하지 못했다. 순수 수치만 보면 잉여율이
크게 개선됐지만(75.7%→30.0% 평균, 29.2%→8.0% 최악), 이는 "마을 상점이
있다"는 잘못된 전제를 걷어내고 "제련소가 사실상 유일한 무제한 소비처"임을
반영한 결과다 — **B5를 해결로 처리하지 않는다.** 마을 상점 도달 불가
문제와 분수 사문화 문제, 둘 다 설계 판단이 필요해 미해결로 유지한다.
